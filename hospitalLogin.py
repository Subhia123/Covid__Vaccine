...
class login:
    ...
    def __init__(self):
        ...
        Label(self.root, text='Hospital Login', font=('arial', 30, 'bold', 'underline')).pack()
    ...
    def checkLogin(self):
        ...
        cr = conn.cursor()
        q = 'select * from hospitalcamp where email="{}" and password="{}"'.format(self.email,self.password)
        cr.execute(q)
        result = cr.fetchone()
        if result == None:
            ...
            self.root.destroy()
            hospitalMain.main(email=self.email,name=result[0])
